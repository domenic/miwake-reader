...
### Batch 9 (Phase 2) — sync-source seeding flake hardening

- **Do not use ambient pushes as setup**: when a scenario only needs "the source already contains the valid book", import the fixture before connecting FS sync and use `syncValidBookFixtureToSource()`. The source-connection mirror is deterministic setup, while importing into an already-connected source depends on the debounced ambient push and can look idle before the source folder has appeared under parallel load.
- **Speed up ambient sync in Playwright only**: the harness init script sets `window.__miwakeTestSyncPushDebounceMs` before app modules load, so the sync engine keeps its 5s production debounce while tests use a short debounce. This is the preferred Phase 2 answer for suites dominated by ambient push waits.
- **Treat hydration races as product bugs**: the SSR app shell is `inert` until the root layout removes the guard after Svelte mount + `tick()`. This prevents real users, and therefore Playwright, from clicking visually-present controls before their handlers exist. Route helpers should not grow per-button retry loops or navigation wrappers to paper over pre-hydration interactivity.

### Batch 10 (Phase 2) — UI-created source progress round trip

- **Do not manufacture OPFS or IDB state for source fixtures**: create source contents by importing real EPUB fixtures through `/manage`, connecting FS sync, and letting the app mirror the data. For source-only placeholder states, use `signOutAndWipe()` and reconnect to the same OPFS-backed source.
- **Exercise progress sync through reader controls**: use the real reader completion button to create progress state, wait for sync, then verify it from a fresh-device flow by opening the placeholder and checking the user-visible "Undo Complete" control.
- **Leave source-mutation-only harness cases unported until they can be made user-driven**: scenarios that require arbitrary `bookdata_*` / `progress_*` filename edits in the source should not be recreated with direct OPFS writes. Prefer a user-visible workflow that creates the same state; otherwise record the scenario as D-class instead of rebuilding the old shortcut.
