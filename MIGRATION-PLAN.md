...
- In-tree Playwright FS specs: 30 files, 32 tests under `tests/integration/fs/`.
...
- Latest verification: `npm run check` and `npx playwright test tests/integration/fs` passed after the statistics/source split.
...
The conflict-resolution/timestamp cluster is the next good batch:

- `27-force-resync-remote-wins-overwrites-newer-local.mjs`
- `28-zip-wins-overwrites-newer-local.mjs`
- `32-newest-import-preserves-timestamp-protection.mjs`
- `40-keep-newest-import-uses-goal-timestamp.mjs`

These all need a clear way to create two UI-distinguishable versions of the same logical data, then verify the selected conflict direction through UI-readable state.
