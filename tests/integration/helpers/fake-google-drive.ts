...
import {
  ...
  fulfillBytes,
  fulfillJSON,
  fakeCloudStorageErrorStatus,
  hasOnlyUniqueQueryParameters,
  pkceChallengeFor
} from './fake-cloud-http.ts';
import {
  bookDataFilePrefix,
  progressFilePrefix,
  readingGoalsFilePrefix
} from './fake-sync-wire.ts';
...
const THUMBNAIL_URL = 'https://fake-google-drive-content.test/thumbnails/**';
...
interface PausedGoogleDriveUpload {
  release(): void;
  started: Promise<void>;
}

interface UploadPause {
  notifyStarted(): void;
...
export class FakeGoogleDrive {
  ...
  readonly #expectedClientIds: ReadonlySet<string> | undefined;
  ...
  #uploadPause: UploadPause | undefined;
  ...
  constructor(snapshot: readonly SyncRootSnapshotEntry[], options: FakeGoogleDriveOptions = {}) {
    this.#expectedClientIds = options.clientIds ? new Set(options.clientIds) : undefined;
  ...
  hasBookData(title: string): boolean {
    return this.bookFileNames(title).some((filename) => filename.startsWith(bookDataFilePrefix));
  }

  progressFileNames(title: string): string[] {
    return this.bookFileNames(title).filter((filename) => filename.startsWith(progressFilePrefix));
  }
  ...
  readingGoalFileNames(): string[] {
    return this.rootFileNames().filter((filename) => filename.startsWith(readingGoalsFilePrefix));
  }
  ...
  readingGoalStartDates(): string[] {
    ...
    for (const item of this.#storage.listChildren(this.#storage.rootId)) {
      if (item.kind !== 'file' || !item.name.startsWith(readingGoalsFilePrefix)) continue;
  ...
  failRefreshes(count = 1): void {
    this.#refreshFailuresRemaining = count;
  }

  pauseNextUpload(): PausedGoogleDriveUpload {
    if (this.#uploadPause) throw new Error('A fake Google Drive upload is already paused');
  ...
  async #authorize(route: Route): Promise<void> {
    ...
    if (redirectURI) {
      try {
        ...
        // A provider cannot safely redirect to an invalid URI.
      }
    }
    ...
    if (route.request().method() !== 'GET') {
      await this.#failAuthorization(
        route,
        redirectURL,
        'Google authorization requests must use GET'
      );
      return;
    }
    ...
    try {
      authorization = parseGoogleAuthorization(requestURL);
      if (this.#expectedClientIds && !this.#expectedClientIds.has(authorization.clientId)) {
        throw new Error('Google authorization received an unexpected client_id');
      }
    } catch (error) {
      await this.#failAuthorization(route, redirectURL, errorMessage(error));
      return;
    }

    const code = `fake-authorization-code-${++this.#authorizationSequence}`;
    this.#authorizationCodes.set(code, authorization);
    const successRedirectURL = new URL(authorization.redirectURI);
    successRedirectURL.searchParams.set('code', code);
    await route.fulfill({
      status: 302,
      headers: { location: successRedirectURL.href }
    });
  }
  ...
  async #exchangeToken(route: Route): Promise<void> {
    ...
    if (grantType === 'authorization_code') {
      ...
      if (!codeVerifier || pkceChallengeFor(codeVerifier) !== authorization.codeChallenge) {
        await this.#failOAuth(route, 'invalid_grant', 'PKCE code verifier does not match');
        return;
      }

      issueRefreshToken = authorization.needsRefreshToken;
      ...
      if (this.#refreshFailuresRemaining > 0) {
        ...
        await this.#failOAuth(
          ...
          'The fake token endpoint is temporarily unavailable.',
          503
        );
        return;
      }
  ...
  async #createFolder(route: Route): Promise<void> {
    ...
    if (
      ...
      !isStringArray(metadata.parents) ||
    ...
    try {
      ...
      await this.#fail(route, errorMessage(error), fakeCloudStorageErrorStatus(error));
    }
  }
  ...
  async #handleUploadRequest(route: Route, requestURL: URL): Promise<void> {
    ...
    const uploadPause = this.#uploadPause;
    if (uploadPause) {
      this.#uploadPause = undefined;
      uploadPause.notifyStarted();
      await uploadPause.released;
    }
  ...
  async #handleThumbnailRequest(route: Route): Promise<void> {
    if (route.request().method() !== 'GET') {
      await this.#fail(route, 'Unsupported fake Drive thumbnail request', 501);
      return;
    }
    const itemId = /^\/thumbnails\/(?<id>[^/=]+)=s\d+$/.exec(
      new URL(route.request().url()).pathname
    )?.groups?.id;
    const item = itemId ? this.#storage.get(itemId) : undefined;
    if (!item || item.kind !== 'file' || !item.name.startsWith('cover_')) {
      await this.#fail(route, 'Fake Drive thumbnail not found', 404);
      return;
    }
    await fulfillBytes(route, item.data, contentTypeFor(item.name));
  }
  ...
  #driveItem(item: FakeCloudItem): object {
    return {
      ...
      ...(item.kind === 'file' && item.name.startsWith('cover_')
        ? {
            thumbnailLink: `https://fake-google-drive-content.test/thumbnails/${item.id}=s220`
          }
        : {}),
  ...
  async #failAuthorization(
    ...
    if (!redirectURL) {
      await this.#failOAuth(route, 'invalid_request', description);
      return;
    }
    redirectURL.searchParams.set('error', 'invalid_request');
    redirectURL.searchParams.set('error_description', `Fake Google OAuth: ${description}`);
    await route.fulfill({ status: 302, headers: { location: redirectURL.href } });
  }

  async #failOAuth(route: Route, error: string, description: string, status = 400): Promise<void> {
    await fulfillJSON(
      route,
...
function parseGoogleAuthorization(requestURL: URL): GoogleAuthorization {
  ...
  const accessType = requestURL.searchParams.get('access_type');
  if (accessType !== null && accessType !== 'offline') {
    throw new Error('Google authorization requires access_type=offline when provided');
  }
  const prompt = requestURL.searchParams.get('prompt');
  if (prompt !== null && prompt !== 'consent') {
    throw new Error('Google authorization requires prompt=consent when provided');
  }

  return {
    clientId,
    codeChallenge,
    needsRefreshToken: accessType === 'offline' && prompt === 'consent',
    redirectURI
  };
}
...
async function parseMultipartUpload(route: Route): Promise<MultipartUpload> {
  ...
  let metadata: { name?: unknown; parents?: unknown } | null;
  try {
...
function isValidDriveListRequest(searchParams: URLSearchParams): boolean {
  if (
    !hasOnlyUniqueQueryParameters(searchParams, ['corpora', 'spaces', 'fields', 'q']) ||
