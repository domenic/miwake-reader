...
import { bookDataFilePrefix, progressFilePrefix } from './fake-sync-wire.ts';
...
export class FakeOneDrive {
  ...
  readonly #pendingThumbnails: boolean;
  ...
  get cancelledUploadSessions(): number {
    return this.#cancelledUploadSessions;
  }

  get failedUploadChunks(): number {
    return this.#failedUploadChunks;
  }

  get latestAccessToken(): string {
    return this.#latestAccessToken;
  }

  get uploadSessionCount(): number {
  ...
  constructor(snapshot: readonly SyncRootSnapshotEntry[], options: FakeOneDriveOptions = {}) {
    this.#expectedClientId = options.clientId;
    this.#clientSecret = options.clientSecret;
    this.#pendingThumbnails = options.pendingThumbnails ?? false;
  ...
  async install(context: BrowserContext): Promise<void> {
    ...
    await context.route(DEFAULT_TOKEN_URL, (route) => this.#exchangeToken(route));
    if (this.#tokenEndpoint !== DEFAULT_TOKEN_URL) {
  ...
  hasBookData(title: string): boolean {
    return this.#storage
      .fileNamesInFolder(title)
      .some((name) => name.startsWith(bookDataFilePrefix));
  }
  ...
  async #authorize(route: Route): Promise<void> {
    ...
    try {
      ...
      await route.fulfill({ status: 400, body: 'Invalid redirect_uri' });
      return;
    }
  ...
  #validateAuthorizationRequest(requestURL: URL): string | undefined {
    ...
    if (this.#expectedClientId !== undefined && clientId !== this.#expectedClientId) {
      return 'Unexpected client_id';
    }
  ...
  async #exchangeToken(route: Route): Promise<void> {
    ...
    if (route.request().url() !== this.#tokenEndpoint) {
      await fulfillJSON(
        route,
        {
          error: 'invalid_request',
          error_description:
            'The default token endpoint was used instead of the configured endpoint.'
        },
        400
      );
      return;
    ...
    if (
      !route.request().headers()['content-type']?.startsWith('application/x-www-form-urlencoded')
    ) {
    ...
    if (grantType === 'authorization_code') {
      ...
      if (
        ...
        pkceChallengeFor(params.get('code_verifier') ?? '') !== codeData.codeChallenge
      ) {
      ...
      refreshToken = this.#issueRefreshToken(codeData.clientId);
    } else {
      const suppliedRefreshToken = params.get('refresh_token') ?? '';
      const refreshClientId = this.#refreshTokens.get(suppliedRefreshToken);
      if (!refreshClientId || refreshClientId !== params.get('client_id')) {
      ...
      this.#refreshTokens.delete(suppliedRefreshToken);
      this.#refreshExchanges += 1;
      refreshToken = this.#issueRefreshToken(refreshClientId);
    }
  ...
  #hasExpectedCredentials(params: URLSearchParams): boolean {
    ...
    if (
      !clientId ||
      (this.#expectedClientId !== undefined && clientId !== this.#expectedClientId)
    ) {
      return false;
    }
  ...
  #issueRefreshToken(clientId: string): string {
  ...
  async #handleBatchRequest(route: Route): Promise<void> {
    ...
    try {
      requests = (parseJSONRequest(route) as { requests?: unknown } | null)?.requests;
    } catch {
    ...
    if (
      ...
      requests.length === 0 ||
      requests.length > 20 ||
      !requests.every(isGraphBatchRequest) ||
  ...
  async #createFolder(route: Route, parentId: string): Promise<void> {
    let body: {
      '@microsoft.graph.conflictBehavior'?: unknown;
      folder?: unknown;
      name?: unknown;
    } | null;
    try {
      body = parseJSONRequest(route) as typeof body;
    } catch {
    ...
    try {
      ...
      await this.#fulfillGraphError(route, errorMessage(error), fakeCloudStorageErrorStatus(error));
    }
  }
  ...
  async #createUploadSession(
    ...
    let body: { item?: { name?: unknown } | null } | null;
    try {
      body = parseJSONRequest(route) as typeof body;
    } catch {
      await this.#fulfillGraphError(route, 'Invalid upload-session body', 400);
      return;
    }
    ...
    if (!this.#isFolder(targetParentId)) {
      await this.#fulfillGraphError(route, 'Upload parent not found', 404);
      return;
    }
  ...
  async #handleUploadRequest(route: Route): Promise<void> {
    ...
    if (request.method() === 'DELETE') {
      ...
      this.#cancelledUploadSessions += 1;
      await route.fulfill({ status: 204, headers: corsHeaders() });
      return;
    }
    ...
    if (this.#failNextUploadChunk) {
      this.#failNextUploadChunk = false;
      this.#failedUploadChunks += 1;
      await fulfillJSON(route, { error: { message: 'Injected upload failure' } }, 503);
      return;
    }
  ...
  #toDriveItem(item: FakeCloudItem, withThumbnails = false) {
    ...
    return {
      ...
      ...(withThumbnails && item.name.startsWith('cover_')
        ? { thumbnails: this.#pendingThumbnails ? [] : [this.#thumbnail(item)] }
        : {})
    };
  }
...
function hasSameUniqueCommaSeparatedValues(actual: string | null, expected: string): boolean {
  ...
  return (
    values.length === new Set(values).size &&
    setsEqual(new Set(values), new Set(commaSeparatedValueList(expected)))
  );
}
