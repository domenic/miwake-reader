...
export class FakeCloudStorage {
  ...
  createFolder(parentId: string, name: string): FakeCloudFolder {
    ...
    if (this.listChildren(parentId).some((item) => item.name === name)) {
      throw new Error(`An item named ${name} already exists under ${parentId}`);
    }
  ...
  putFile(
    ...
    if (existingId) {
      ...
      if (!existing || existing.kind !== 'file') {
        throw new Error(`Cannot update missing fake cloud file ${existingId}`);
      }
      if (existing.parentId !== parentId) {
        throw new Error(`Cannot move fake cloud file ${existingId} while updating it`);
      }
      existing.name = name;
  ...
  rename(id: string, name: string): FakeCloudItem {
    ...
    if (!item) throw new Error(`Cannot rename missing fake cloud item ${id}`);
    if (
      this.listChildren(item.parentId).some(
        (candidate) => candidate.id !== id && candidate.name === name
      )
    ) {
      throw new Error(`An item named ${name} already exists under ${item.parentId}`);
    }
    item.name = name;
    return item;
  }
  ...
  #requireFolderParent(parentId: string): void {
    ...
    if (!parent || parent.kind !== 'folder') {
      throw new Error(`Missing fake cloud parent folder ${parentId}`);
    }
  }
}
