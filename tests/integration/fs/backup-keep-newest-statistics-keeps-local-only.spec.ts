...
const ZIP_STAT_DATE = '2026-04-10';
const LOCAL_ONLY_STAT_DATE = '2026-04-15';

test('backup import with "Keep newest" keeps local-only statistics', async ({ page }, testInfo) => {
  const backupPath = testInfo.outputPath('statistics-backup.zip');

  await page.clock.install({ time: new Date(`${ZIP_STAT_DATE}T11:59:00Z`) });
  await enableStatistics(page);
  await importBookFixtures(page, [VALID_BOOK]);
  await recordStatisticForBook(page, VALID_BOOK, ZIP_STAT_DATE);
  ...
  await expectStatisticsInSummary(page, {
    present: [
      { fixture: VALID_BOOK, dateKey: ZIP_STAT_DATE },
