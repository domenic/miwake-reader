...
import { setReadingGoal, waitForSuccessfulSync } from '../helpers/workflows.ts';
...
test('creating and deleting reading goals rewrites Google Drive root data', async ({
  ...
  await page.clock.install({ time: new Date('2026-05-22T12:00:00Z') });
  const fakeDrive = await connectToEmptyFakeGoogleDrive(context, page);
  expect(fakeDrive.rootFileNames()).toEqual([]);

  await setReadingGoal(page, { timeGoal: '30', startDate: '2026-05-22' });
  await expect
    .poll(() => readingGoalFileNames(fakeDrive), { timeout: SYNC_ASSERTION_TIMEOUT })
    .toHaveLength(1);
  expect(fakeDrive.readingGoalStartDates()).toEqual(['2026-05-22']);
  await waitForSuccessfulSync(page);
  const [createdGoalFilename] = readingGoalFileNames(fakeDrive);

  await page.clock.setSystemTime(new Date('2026-05-23T12:00:00Z'));
  ...
  await expect
    .poll(
      () => {
        const filenames = readingGoalFileNames(fakeDrive);
        return {
          count: filenames.length,
...
function readingGoalFileNames(fakeDrive: FakeGoogleDrive): string[] {
  return fakeDrive.rootFileNames().filter((filename) => filename.startsWith('miwake-user-goals_'));
}
